...
std::string CloudPluginDownloader::download_from_cloud(PluginType plugin_type,
    ...
    try {
        ...
        if (!expected_md5.empty()) {
            ...
            LOG(ERROR) << "Failed to download plugin " << plugin_name << " with user-provided MD5 "
                       << expected_md5 << ": " << e.what();
            throw std::runtime_error("Failed to download plugin " + plugin_name +
                                     " with expected MD5 " + expected_md5 + ": " + e.what());
        } else {
