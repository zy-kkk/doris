...
std::string CloudPluginDownloader::build_cloud_path(PluginType plugin_type,
    ...
    // TODO: In FE, it first tries ((CloudEnv) Env.getCurrentEnv()).getCloudInstanceId()
    // then falls back to Config.cloud_unique_id. For now, we use cloud_unique_id directly.
    std::string instance_id = config::cloud_unique_id;
...
bool CloudPluginDownloader::is_local_file_up_to_date(const std::string& remote_path,
    ...
    try {
        ...
        if (remote_etag.empty()) {
            ...
            if (file_age.count() > 1) {
                LOG(INFO) << "Local file " << local_path << " is older than 1 hour, may need update";
                return false;
            }
            return true;
        }
        ...
        try {
            ...
            if (cached_etag == remote_etag) {
                LOG(INFO) << "Local file " << local_path << " is up to date (ETag match: " 
                          << remote_etag << ")";
                return true;
            } else {
