...
public class HiveMetaStoreCache {
    ...
    public static class FileCacheKey {
        ...
        public long getRetainedSizeInBytes() {
            return INSTANCE_SIZE
                    ...
                    + SizeOf.estimatedSizeOf(partitionValues, s -> SizeOf.estimatedSizeOf(s));
        }
    }
    ...
    public static class FileCacheValue {
        ...
        public long getRetainedSizeInBytes() {
            ...
            // files list
            size += SizeOf.estimatedSizeOf(files, HiveFileStatus::getRetainedSizeInBytes);
            // partitionValues
            size += SizeOf.estimatedSizeOf(partitionValues, s -> SizeOf.estimatedSizeOf(s));
            // AcidInfo (estimated)
            if (acidInfo != null) {
                size += SizeOf.estimatedSizeOf(acidInfo.getPartitionLocation());
                if (acidInfo.getDeleteDeltas() != null) {
                    size += 100L * acidInfo.getDeleteDeltas().size();
                }
            }
            return size;
        }
    ...
    public static class HiveFileStatus {
        ...
        // Estimated size per BlockLocation: ~200 bytes (hosts[], names[], offset, length, etc.)
        ...
        public long getRetainedSizeInBytes() {
            ...
            // LocationPath contains normalizedLocation string
            if (path != null) {
            ...
            if (acidInfo != null) {
                size += SizeOf.estimatedSizeOf(acidInfo.getPartitionLocation());
                if (acidInfo.getDeleteDeltas() != null) {
                    size += 100L * acidInfo.getDeleteDeltas().size(); // rough estimate per delta
                }
            }
            return size;
        }
