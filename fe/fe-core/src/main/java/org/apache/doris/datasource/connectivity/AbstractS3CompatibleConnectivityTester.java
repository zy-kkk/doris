...
public abstract class AbstractS3CompatibleConnectivityTester implements StorageConnectivityTester {
    ...
    @Override
    public final void testFeConnection() throws Exception {
        if (shouldSkipTest()) {
            return;
        }
        doTestFeConnection();
    ...
    @Override
    public final void testBeConnection() throws Exception {
        if (shouldSkipTest()) {
            return;
        }
    ...
    /**
     * Perform actual FE connection test. Subclass should implement this method.
     * This method will only be called when shouldSkipTest() returns false.
     */
