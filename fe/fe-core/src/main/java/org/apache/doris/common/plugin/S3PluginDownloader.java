...
public class S3PluginDownloader implements AutoCloseable {
    ...
    private String doDownloadFile(String remoteS3Path, String localPath, String expectedMd5) throws Exception {
        ...
        if (PluginFileCache.isFileValid(localPath, expectedMd5)) {
            ...
            if (!Strings.isNullOrEmpty(expectedMd5)) {
                ...
                if (!hasRemoteUpdate(remoteS3Path, localPath)) {
                    LOG.info("Local file {} MD5 validated and remote is consistent, skipping download", localPath);
                    return localPath;
                } else {
                    LOG.info("Local file {} MD5 valid, but remote has different version, downloading to verify",
                            localPath);
                }
            } else {
                // No user MD5, check remote only if cache expired (optimization)
                if (!PluginFileCache.needsRemoteCheck(localPath, expectedMd5)
                        || !hasRemoteUpdate(remoteS3Path, localPath)) {
                    LOG.info("Local file {} is up to date (cache optimization), skipping download", localPath);
                    return localPath;
                }
            }
        }
        ...
        updateCacheAfterDownload(remoteS3Path, localPath);

        LOG.info("Successfully downloaded {} to {}", remoteS3Path, localPath);
        return localPath;
    }

    // Note: isLocalFileValid method removed, now using PluginFileCache.validateFile instead
    ...
    /**
     * Simple remote update check - only when cache expired
     */
    private boolean hasRemoteUpdate(String remoteS3Path, String localPath) {
        try {
            String remoteEtag = s3Storage.getObjectETag(remoteS3Path);
            return PluginFileCache.hasRemoteUpdate(localPath, remoteEtag, 0);
        } catch (Exception e) {
            LOG.debug("Failed to check remote update for {}: {}", remoteS3Path, e.getMessage());
     ...
     */
    private void updateCacheAfterDownload(String remoteS3Path, String localPath) {
        try {
            String remoteEtag = s3Storage.getObjectETag(remoteS3Path);
            File localFile = new File(localPath);
            long fileSize = localFile.exists() ? localFile.length() : 0;

            PluginFileCache.updateCache(localPath, remoteEtag, fileSize);
        } catch (Exception e) {
            LOG.debug("Failed to update cache for {}: {}", localPath, e.getMessage());
        }
    }
