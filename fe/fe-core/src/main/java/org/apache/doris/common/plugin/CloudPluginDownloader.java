...
public class CloudPluginDownloader {
    ...
    public static String downloadFromCloud(PluginType pluginType, String pluginName,
        ...
        try {
            ...
            // Always handle error properly based on user intent
            if (!Strings.isNullOrEmpty(expectedMd5)) {
                // User provided MD5 - this is a hard requirement, must throw error
                LOG.error("Failed to download plugin {} with user-provided MD5 {}: {}",
                        pluginName, expectedMd5, e.getMessage());
                throw new IllegalArgumentException("Failed to download plugin " + pluginName
                        + " with expected MD5 " + expectedMd5 + ": " + e.getMessage(), e);
            } else {
                // User didn't provide MD5 - this is optional, return empty for fallback
