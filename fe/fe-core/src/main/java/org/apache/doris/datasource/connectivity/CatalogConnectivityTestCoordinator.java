...
public class CatalogConnectivityTestCoordinator {
    ...
    public void runTests() throws DdlException {
        ...
        String warehouse = testMetadataService();

        // 2. Test object storage if warehouse is available
        if (StringUtils.isNotBlank(warehouse)) {
            testObjectStorageForWarehouse(warehouse);
        }

        // 3. Test explicitly configured HDFS
     ...
     *
     * @return warehouse location if available, null otherwise
     * @throws DdlException if test fails
     */
    private String testMetadataService() throws DdlException {
        ...
        String warehouse = metaTester.getTestLocation();
        if (StringUtils.isNotBlank(warehouse)) {
            LOG.debug("Got warehouse location from metadata service: {}", warehouse);
        }
    ...
    /**
     * Test object storage that matches the warehouse location.
     *
     * @param warehouse warehouse location (e.g., s3://bucket/path)
     * @throws DdlException if test fails
     */
    private void testObjectStorageForWarehouse(String warehouse) throws DdlException {
        StorageProperties matchedStorage = findMatchingObjectStorage(warehouse);

        if (matchedStorage == null) {
            LOG.debug("No object storage configured for warehouse '{}' in catalog '{}'",
                    warehouse, catalogName);
            return;
        }

        LOG.info("Testing {} connectivity for warehouse '{}' in catalog '{}'",
                matchedStorage.getStorageName(), warehouse, catalogName);

        // Create tester with the warehouse location
        StorageConnectivityTester tester = createStorageTester(matchedStorage, warehouse);
    ...
    private void testExplicitlyConfiguredHdfs() throws DdlException {
        ...
        // Skip if not explicitly configured by user
        if (!hdfs.isExplicitlyConfigured()) {
            LOG.debug("Skip HDFS connectivity test for catalog '{}': "
                    + "HDFS is not explicitly configured by user", catalogName);
            return;
        }

        // Skip if no fs.defaultFS configured
        String defaultFS = hdfs.getDefaultFS();
        if (StringUtils.isBlank(defaultFS)) {
            LOG.debug("Skip HDFS connectivity test for catalog '{}': "
                    + "fs.defaultFS is not configured", catalogName);
            return;
        }
        ...
        try {
            ...
            throw new DdlException("HDFS connectivity test failed (compute node): "
                    + e.getMessage());
        }
    }
    ...
    private StorageConnectivityTester createStorageTester(StorageProperties props, String location) {
        ...
        if (props instanceof HdfsProperties) {
            return new HdfsConnectivityTester((HdfsProperties) props, location);
        }
