...
public class TrinoConnectorJniWriter extends JniScanner {
    ...
    private io.trino.spi.connector.ConnectorMetadata metadata;
    ...
    public void open() throws IOException {
        try {
            ...
            try {
                ...
                // 使用beginInsert方法获取InsertTableHandle
    ...
    private void parseRequiredTypes() {
        // 如果fields为空，则从columnMetadataList初始化
        if (fields == null || fields.length == 0) {
            fields = trinoConnectorAllFieldNames.toArray(new String[0]);
        }
    ...
    public void finishWrite() throws Exception {
        ...
        try {
            // 调用sink的finish方法，获取fragments（返回的是CompletableFuture）
                ...
                try {
                    ...
                    java.util.Optional<io.trino.spi.connector.ConnectorOutputMetadata> outputMetadata =
                            metadata.finishInsert(
                                    ...
                                    insertTableHandle,
                                    fragments,
                                    java.util.Collections.emptyList()); // computedStatistics
    ...
    // 添加一个辅助方法来获取列名
    private String getColumnName(VectorTable table, int index) {
